...
const ContactPage = () => {
  ...
  const [showPopUp, setShowPopUp] = useState({ show: false, text: "Message Sent!" });
    ...
    if (contactInput.name && contactInput.email && contactInput.title && contactInput.message) {
      emailjs.send("service_f904uuc", "template_xp3618s", { name: contactInput.name, email: contactInput.email, title: contactInput.title, message: contactInput.message }, "WlCoMP40HZDFpDPbX").then(
        (response) => {
          ...
          setShowPopUp({ text: "Message Sent!", show: true });
        },
        (error) => {
          console.log("FAILED...", error);
          setShowPopUp({ text: "Message Failed!", show: true });
        },
      );
    } else {
      setContactInputStyle({ name: `placeholder:text-${contactInput.name ? "gray" : "red"}-400`, email: `placeholder:text-${contactInput.email ? "gray" : "red"}-400`, title: `placeholder:text-${contactInput.title ? "gray" : "red"}-400`, message: `placeholder:text-${contactInput.message ? "gray" : "red"}-400` });
    }
  };
      ...
      {showPopUp.show && (
        <motion.main initial={{ x: 500 }} animate={{ x: 0, transition: { duration: 1, damping: 9, type: "spring", ease: "easeInOut" } }} className="fixed w-full top-0 right-0 md:w-fit md:top-6 md:right-6 lg:top-4 lg:right-10">
          <div className="relative py-6 md:py-8 md:px-20 lg:py-6 bg-neutral-800/50 backdrop-blur-sm text-gray-400 md:rounded-tl-3xl md:rounded-br-3xl">
            <p className="text-center text-xs md:text-xl lg:text-base">{showPopUp.text}</p>
            <FaX className="absolute top-3 right-3 md:top-4 md:right-4 lg:top-2 lg:right-2 text-white text-xs md:text-base cursor-pointer" onClick={() => setShowPopUp({ ...showPopUp, show: false })} />
          </div>
        </motion.main>
      )}
