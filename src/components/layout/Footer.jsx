  ...
  return (
    <footer className={`relative p-4 md:py-8 md:px-10 lg:py-5 lg:px-16`}>
      <div className={`${location !== "/" ? "invisible" : ""} md:visible flex items-center gap-x-5 md:gap-x-10 lg:gap-x-7`}>
        {socialsPages.map(({ id, text, icon, socialsLink }) => <a key={id} target="_blank" aria-label={text} href={socialsLink}><FontAwesomeIcon icon={icon} className="text-gray-400 text-base md:text-2xl lg:text-xl transition-all ease-linear duration-200 hover:scale-125" /></a>)}
      </div>
