...
const PageNavigation = () => {
  ...
  return (
    <>
      <div onClick={() => setShowNavigation(!showNavigation)} className="relative group select-none md:hidden cursor-pointer mr-4 md:m-0 border-2 border-gray-400 transition-all ease-linear duration-200 hover:border-white rounded-md w-8 h-8">
      ...
      <nav className={`${showNavigation ? "flex" : "hidden"} z-20 md:flex select-none absolute bg-dark-semi md:bg-transparent flex-col py-2 md:p-0 top-full md:top-0 w-full md:w-fit md:relative md:flex-row md:gap-x-6 lg:gap-x-16`}>
