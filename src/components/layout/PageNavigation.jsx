...
const PageNavigation = () => {
  ...
  const Nav = memo(({ className, children, initial, style, animate, exit, transition }) => {
    return (
  ...
  const HamburgerButton = memo(({ isOpen, toggle }) => {
    return (
      <MotionConfig transition={{ duration: 0.35, ease: "easeInOut" }}>
        <motion.button onClick={toggle} animate={isOpen ? "open" : "close"} className="relative md:hidden h-8 w-8 mr-4 rounded-md border-2 border-gray-400">
